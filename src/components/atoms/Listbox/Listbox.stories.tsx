...
import React, {Key, SVGProps, FC} from "react";
...
const WithEndContentTemplate = ({color, variant, disableAnimation, ...args}: ListboxProps) => {
...
const WithTopContentTemplate = ({color, ...args}: ListboxProps<{id: number, name: string, email: string, avatar: string}>) => {
...
const WithBottomContentTemplate = ({color, ...args}: ListboxProps<{id: number, name: string, email: string, avatar: string}>) => {
  const [values, setValues] = React.useState<Selection>(new Set(["1", "3", "5"]));
...
const WithDescriptionTemplate = ({color, variant, disableAnimation, ...args}: ListboxProps) => {
  const iconClasses = "text-2xl text-secondary pointer-events-none flex-shrink-0";
...
const WithSectionsTemplate = ({color, variant, disableAnimation, ...args}: ListboxProps) => {
  const iconClasses = "text-2xl text-secondary pointer-events-none flex-shrink-0";
...
const CustomWithClassNamesTemplate = ({color, variant, disableAnimation, ...args}: ListboxProps) => {
